...
const Todo = ({task, setModalTitle, handleDeleteTask, handleUpdateTask}) => {
    ...
    <>
        <div className={`bg-base-100 shadow-xl border-secondary border-solid border-2 rounded-md text-left p-3 mb-3 ${task.is_done ? "line-through": ""}`}>
            ...
            <button className="btn btn-success btn-sm mr-2" onClick={toggleChangeStatus}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            ...
            <DeleteConfirmation item={task} handleDelete={handleDeleteTask} />
        </div>
        </div>
    </>
    )
